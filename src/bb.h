...
#define __BB_INIT(SCOPE, bbtype, prefix, buf_t)         \
	SCOPE int prefix##_init(bbtype *bb, size_t cap) \
	{                                               \
		bb->cap = cap;                          \
		bb->len = 0;                            \
		bb->buf = xmalloc(cap);                 \
...
#define __BB_CLEAR(SCOPE, bbtype, prefix)     \
	SCOPE void prefix##_clear(bbtype *bb) \
	{                                     \
		bb->len = 0;                  \
	}

#define __BB_REVERSE(SCOPE, bbtype, prefix, buf_t)          \
	SCOPE int prefix##_reverse(bbtype *bb, size_t need) \
	{                                                   \
...
#define __BB_APPEND(SCOPE, bbtype, prefix, buf_t)                        \
	SCOPE int prefix##_append(bbtype *bb, const void *src, size_t n) \
	{                                                                \
		if (prefix##_reverse(bb, n) < 0)                         \
			return -1;                                       \
